...
import { Button, Container, Comment, Loader, Icon } from "semantic-ui-react";
...
export default observer(function ConnectMessage() {
    ...
    const { deleteLoading } = messageStore;
    const [previewEmoji, setPreviewEmoji] = useState(false);
    const [emojiValue, setEmojiValue] = useState<string>("");

    useEffect(() => {
        if (user && member) {
            messageStore.createHubConnection(member.username);
        }
        return () => {
    ...
    useEffect(() => {
        setEmojiValue((prev: string) =>
            prev === emojiValue ? "" : emojiValue
        );
    }, [emojiValue]);
	...
	if (deleteLoading)
		return <LoadingComponent inverted content="Loading messages..." />;
            ...
            <div className="mesageForm">
                <Formik
                    onSubmit={(values, { resetForm }) => {
                        messageStore
                            .sendMessage(member.username, values.body)
                            .then(() => resetForm());
                    }}
                    initialValues={{ body: "" }}
